...
- [ ] **Step 2: Run** — Expected: FAIL.
- [ ] **Step 3: Implement** `begin`: generate `OAuthPKCE`; if `forcePaste`, build a `.paste` PendingLogin with `redirectURI = OAuthEndpoints.pasteRedirect`, server nil; else construct a `LoopbackServer`, `try start()` → on success `.loopback(port:)` with **`redirectURI = "http://localhost:\(port)/callback"`**, on `StartError.bindFailed` fall back to paste.
  ...
  - **Ordering the API cannot enforce:** `waitForCallback` must be armed BEFORE the browser is opened. Until the wait arms the listener, a callback is answered `404` like any stray request, burning a single-use code. Structure `beginLogin` so the wait is started first, then `openURL`. Return the pending, the authorize URL **built inside `begin`** (it takes `loginHintEmail` as its third parameter and forwards it — the caller must NOT rebuild the URL), the server, and the already-armed `callback` task. **Do not open the browser here** — the view model opens via injected `openURL` (testability).
- [ ] **Step 4: Run** — Expected: PASS.
...
- [ ] **Step 3: Implement** `beginLogin`:
  ...
  - Loopback: `await result.callback?.value`. nil → **`await result.server?.stop()` first**, then restart as paste: `try await deps.beginLogin(accountID, true, emailFor(accountID))`, set `.awaitingPaste`, open the new URL. Non-nil → `finishLogin(code:)`.
  - **Server-lifetime invariant (decided after the Task 7 re-review — one rule, no exceptions):** `begin`'s callback Task stops the server on SUCCESS. **Task 9 stops it on every other terminal outcome** — timeout-restart, cancel, and any failure that abandons the login. Never drop a `server` reference without stopping it.
    - Why: `LoopbackServer.graceTask` captures `self` weakly and nothing in `deinit` cancels the listener, so dropping the last reference after a timeout releases a still-bound `NWListener` and leaks the port. Two leaks of exactly this class have already been fixed in this feature; a caller obligation to *retain* for the full 10-minute grace period would invite a third.
    - Accepted cost: a browser that redirects after the timeout gets connection-refused instead of Task 6's "login expired" page. That page is cosmetic in an already-failed flow, and the app itself tells the user what happened. Task 6's grace machinery still serves during the brief window before Task 9's stop.
  - **Normalize the login hint:** pass `nil`, never `""`. `begin` forwards an empty string verbatim and would emit a bare `login_hint=`. If `emailFor(accountID)` can return an empty string, map it to `nil` at that seam.
