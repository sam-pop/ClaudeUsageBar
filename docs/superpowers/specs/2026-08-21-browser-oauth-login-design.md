...
- **S3 loopback ports are port-agnostic.** Two distinct ephemeral ports (50832, 50958) both succeeded with `http://localhost:<port>/callback` → any loopback port is accepted (RFC 8252 §7.3).
  - **⚠️ CORRECTION (found in live QA, 2026-08-21).** This entry originally also claimed "`http://127.0.0.1/callback` is registered; both forms work", sourced from the client metadata at `https://claude.ai/oauth/claude-code-client-metadata`. **That is false for the client this app uses.** A real login with the IP literal fails: *"Redirect URI http://127.0.0.1:55250/callback is not supported by client."* The metadata document describes the **URL-form client id**, not the UUID client `9d1c250a-…` — a reviewer flagged that distinction at the time and it was treated as corroborating proof anyway. **Only `localhost` is usable**, which is exactly the form S1/S6 actually exercised. Lesson: a spike-proven value outranks documentation about a neighbouring client.
